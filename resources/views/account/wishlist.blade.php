                    ...
                    <div class="card-luxe overflow-hidden" x-data="{ removed: false }" x-show="!removed">
                        ...
                        <div class="p-2.5">
                            ...
                            <div class="mt-2 flex gap-1.5">
                                <button @click="$store.ui.addToCart({{ Illuminate\Support\Js::from($product['name']) }})" class="btn-primary flex-1 !py-1.5 !px-2 text-[9.5px]" @if(!$product['in_stock']) disabled @endif>Add to Cart</button>
                                <button @click="removed = true; $store.ui.notify('Removed from wishlist')" class="icon-btn h-8 w-8 flex-shrink-0 border border-line text-muted hover:text-error" aria-label="Remove">
