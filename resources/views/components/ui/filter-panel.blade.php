...
@php
    $sections ??= [
        ...
        'type' => ['label' => 'Jewellery Type', 'type' => 'checkbox', 'options' => ['Ring' => 'Ring', 'Earrings' => 'Earrings', 'Necklace' => 'Necklace', 'Pendant' => 'Pendant', 'Bracelet' => 'Bracelet', 'Bangle' => 'Bangle', 'Chain' => 'Chain']],
        'metal' => ['label' => 'Material Type', 'type' => 'checkbox', 'options' => ['Gold' => 'Gold', 'Rose Gold' => 'Rose Gold', 'White Gold' => 'White Gold', 'Silver' => 'Silver', 'Platinum' => 'Platinum']],
...
<div class="divide-y divide-line">
    ...
    <div class="py-4" x-data="{ open: true }">
        ...
        <div x-cloak x-show="open" x-collapse class="mt-4 space-y-3">
            <input type="range" x-model.number="{{ $model }}.priceMax" min="{{ (int) $priceMin }}" max="{{ (int) $priceMax }}" step="1000" class="w-full accent-champagne-dark">
        ...
        <div class="py-4" x-data="{ open: {{ $key === 'category' ? 'true' : 'false' }} }">
            ...
            <div x-cloak x-show="open" x-collapse class="mt-3 space-y-2.5">
                @foreach ($section['options'] as $value => $label)
                    @php
                        $optionLabel = is_array($label) ? ($label['label'] ?? $value) : $label;
                        $optionCount = is_array($label) ? ($label['count'] ?? null) : null;
                    @endphp
                    <label class="flex cursor-pointer items-center gap-2.5 text-[13.5px] text-charcoal-soft">
                        ...
                        @if ($optionCount !== null)
                            <span class="rounded-full bg-ivory-soft px-2 py-0.5 text-[11px] text-muted">{{ $optionCount }}</span>
                        @endif
                    </label>
                @endforeach
            </div>
