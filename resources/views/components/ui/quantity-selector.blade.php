...
<div class="inline-flex items-center rounded-full border border-line">
    <button type="button" @click="{{ $model }} = Math.max({{ $min }}, {{ $model }} - 1)" class="flex h-9 w-9 items-center justify-center text-charcoal transition-colors hover:text-champagne-dark disabled:opacity-30" :disabled="{{ $model }} <= {{ $min }}">
    ...
    <button type="button" @click="{{ $model }} = Math.min({{ $max }}, {{ $model }} + 1)" class="flex h-9 w-9 items-center justify-center text-charcoal transition-colors hover:text-champagne-dark disabled:opacity-30" :disabled="{{ $model }} >= {{ $max }}">
